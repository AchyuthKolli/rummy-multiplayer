...
/* ----- MeldSlotBox and LeftoverSlotBox unchanged (kept for brevity, you can keep your version) ----- */
/* I left these unchanged except minor console logs in your earlier version — not modifying UI. */
...
const MeldSlotBox = ({ title, slots, setSlots, myRound, setMyRound, isLocked = false, onToggleLock, tableId, onRefresh, hideLockButton, gameMode }: MeldSlotBoxProps) => {
  ...
  return (
    <>
      <div className={`border border-dashed rounded p-2 ${isLocked ? 'border-amber-500/50 bg-amber-900/20' : 'border-purple-500/30 bg-purple-900/10'}`}>
...
export default function Table() {
  ...
  // Meld lock state
  const [meldLocks, setMeldLocks] = useState<{ meld1: boolean; meld2: boolean; meld3: boolean; leftover: boolean; }>({ meld1: false, meld2: false, meld3: false, leftover: false });
      ...
      try {
        ...
        if (prev && prev.length > currentIds.length) {
          ...
          leftIds.forEach(async (uid) => {
            ...
            // Best-effort: call a server endpoint to penalize/remove leaving player if it exists.
            try {
              // If your backend exposes an endpoint like penalize_leave, it will be used.
...
if (data.wild_joker_rank) {
  ...
  const onStart = async () => {
    ...
    setStarting(true);
    try {
      // best-effort: include deck_count if API supports it
      const deck_count = determineDecksForPlayers(info.players.length);
      const body: any = { table_id: tableId };
      // If backend supports deck_count we'll include it (safe if backend ignores extra fields)
      body.deck_count = deck_count;
  ...
  const onDrawStock = async () => {
    ...
    try {
      ...
      const data = await res.json();
      // update myRound with server response (keeps discard_top and stock_count in sync)
      setMyRound(data);
      // Determine new card by comparing previous hand (best-effort)
      try {
        const prevHand = myRound?.hand || [];
        const newCard = (data.hand || []).find((card: any) =>
          !prevHand.some((c: any) => c.rank === card.rank && c.suit === card.suit)
        );
        if (newCard) {
...
setTimeout(refresh, 120);
  ...
  const onDrawDiscard = async () => {
    ...
    try {
      ...
      const data = await res.json();
      // Server response should include the updated hand & discard_top.
      setMyRound(data);
      // The drawn card is typically the card that was on discard_top before draw — server may provide it.
      // Try to infer: find newly added card compared to previous hand.
      try {
        const prevHand = myRound?.hand || [];
        const newCard = (data.hand || []).find((card: any) =>
          !prevHand.some((c: any) => c.rank === card.rank && c.suit === card.suit)
        ...
        if (newCard) {
          ...
          // Fallback: parse previous discard_top (the one we took)
          const code = myRound.discard_top;
          if (code === 'JOKER') {
            setLastDrawnCard({ rank: 'JOKER', suit: null });
          } else {
...
setTimeout(refresh, 120);
      ...
      // update myRound if server returned round state
      if (data && data.hand) {
        setMyRound(data);
      }
      // Always refresh to sync discard_top & turn
      await refresh();
    } catch (e: any) {
...
socket.emit("declare_made", { tableId });
  ...
  const onNextRound = async () => {
    ...
    setStarting(true);
    try {
      // best-effort: rotate who picks first (if possible)
      const players = info.players || [];
      // Determine current first picker id - try info.first_player_id or fallback to active_user_id or host
      ...
      if (firstPlayerId && players.length > 0) {
        ...
        if (idx >= 0) {
          ...
          // fallback rotate from host position
      ...
      const body: any = { table_id: tableId };
      // Try sending nextFirstPlayerId if API supports it, harmless if ignored
      body.first_player_id = nextFirstPlayerId;
  ...
  // penalty rule applies only when players > 2 (as requested)
  ...
  // Voice control handler unchanged
...
{revealedWildJoker && (
  <div className="mt-2 px-3 py-1 bg-yellow-900/30 border border-yellow-500/40 rounded-lg text-yellow-300 font-bold text-lg">
    Wild Joker: {revealedWildJoker}
                  ...
                  {/* --- many portions unchanged: round info, table color picker, casino table, meld UI, hand etc --- */}
                  {/* For brevity: keep your existing rendering for these sections (unchanged). */}
                  {/* The rest of your render remains the same; unchanged UI. */}
                  {/* (I didn't change any UI except the small fixes above.) */}
                                ...
                                {melds && melds.length > 0 ? (
                                  <div className="space-y-3">
                                    {melds.map((meld: any, idx: number) => {
                                      ...
                                      if (meldType === "pure") { bgColor = "bg-blue-900/40"; borderColor = "border-blue-500"; label = "Pure Sequence"; }
                                      else if (meldType === "impure") { bgColor = "bg-purple-900/40"; borderColor = "border-purple-500"; label = "Impure Sequence"; }
                                      else if (meldType === "set") { bgColor = "bg-orange-900/40"; borderColor = "border-orange-500"; label = "Set"; }
                              ...
                              <table className="w-full text-xs">
                                <thead>
                                  <tr className="border-b border-border">
                                    ...
                                    {roundHistory.map((round, idx) => <th key={idx} className="text-center py-2 px-1 font-semibold text-foreground">R{round.round_number}</th>)}
